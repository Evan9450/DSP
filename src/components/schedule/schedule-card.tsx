...
const statusConfig: Record<ScheduleStatus, { label: string; className: string }> = {
  ...
  confirmed: { label: 'Confirmed', className: 'bg-blue-600 hover:bg-blue-700' },
  completed: { label: 'Completed', className: 'bg-green-600 hover:bg-green-700' },
};
  ...
  return (
    <Card
      ...
      className={`p-3 cursor-pointer hover:shadow-md transition-all ${
        isHighlighted ? 'ring-2 ring-orange-500 bg-orange-50' : 'bg-white'
      }`}
    >
      <div className="space-y-2">
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-600 ml-6">{schedule.amazonId}</p>
          </div>
        ...
        {schedule.vehicleNumber && (
          <div className="flex items-center gap-2 ml-6">
            ...
            <p className="text-xs font-medium text-gray-700">{schedule.vehicleNumber}</p>
          </div>
        )}
        ...
        {isHighlighted && (
          <div className="mt-2 pt-2 border-t border-orange-200">
            <p className="text-xs font-medium text-orange-700">
              {schedule.isNew ? '🆕 New from Deputy' : '✏️ Modified'}
            </p>
          </div>
        )}
      </div>
